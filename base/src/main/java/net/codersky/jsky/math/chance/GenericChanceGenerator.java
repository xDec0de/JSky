...
public class GenericChanceGenerator<T> implements ChanceGenerator<T> {
    ...
    /**
     * Works in a similar way to {@link #generate()} but limits the generated elements to a {@code max} amount. Keep in
     * mind that the rarest elements take priority, so for example if we have two elements, "Common" (100% chance) and
     * "Rare" (25% chance) and the {@code max} amount to generate is 1, if the 25% chance of getting the "Rare" element
     * was successful, the list will only contain "Rare" as it has a higher priority to be generated than "Common".
     *

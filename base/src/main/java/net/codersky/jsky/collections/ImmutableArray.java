...
/**
 * Array wrapper to make it unmodifiable while keeping memory usage to a minimum. This class only stores the array
...
public class ImmutableArray<E> implements Iterable<E> {
    ...
    /**
     * Creates a new {@link ImmutableArray} that holds the provided {@code array}. The {@code array} <b>WON'T</b> be
     * cloned by the constructor, so avoid modifying it directly.
     *
     * @param array the array to store, can contain {@code null} {@link E elements}.
     *
     ...
     *
     * @param dest The destination array to contain mapped elements on. This array must be big enough to fit all
     * elements, otherwise an {@link ArrayIndexOutOfBoundsException} will be thrown.
     * @param mapper The {@link Function} used to map the elements.
     * @param <R> The type of elements of {@code dest}.
     *
     * @return Just {@code dest}.
     *
     * @throws ArrayIndexOutOfBoundsException if the length of {@code dest} is smaller than {@link #length()}.
     * @since JSky 1.0.0
     */
    @NotNull
     ...
     *
     * @return A new {@link ArrayList} of size {@link #length()} containing all mapped elements.
     *
     * @since JSky 1.0.0
     */
    @NotNull
    ...
    /**
     * Maps every element of this array into a new {@link ArrayList}. Then, adds any {@code extra} elements to it.
     *
     * @param mapper The {@link Function} used to map the elements.
     * @param extra The extra elements to add to the new {@link ArrayList}.
     * @param <R> The type of elements of the new {@link ArrayList}.
     *
     * @return A new {@link ArrayList} of size {@link #length()} + the length of {@code extra} that contains all mapped
     * elements plus those provided at {@code extra}.
     *
     * @since JSky 1.0.0
     */
    @NotNull

...
/**
 * Class used to replace parts of a {@link String} with other objects. This class is designed to make text replacement
 * easy, providing the ability to apply the same replacements on multiple Strings.
 * <p>
 * The most useful feature about the Replacer {@code class} is that it preserves the instance of objects used as
 * replacements, meaning that if the state of a replacement object changes, the change will be shown the next time the
 * Replacer is applied.
 * <p>
 * Also, check the {@link Replacement} {@code interface}. {@link Replacement#asReplacement()} is used instead of
...
public class Replacer implements Cloneable {
    ...
    /**
     * Creates a new {@link Replacer} with the specified {@code replacements}. Keep in mind that the size of the
     * {@code replacements} <b>must be even</b>.
     *
     * @param replacements The replacements to use. This parameter will be translated to a {@link HashMap map}, so the
     * format goes as: [key1, value1, key2, value2...]. The keys are the text to replace, while the values are the
     * replacements. So for example ["%n", 42], would replace "%n" with "42". Keep in mind that as a {@link HashMap map}
     * is used internally, new replacements for the same key will overwrite its value. {@code null} values will be added
     * as the string literal <i>"null"</i>.
     *
     * @throws IllegalArgumentException If the amount of {@code replacements} is not even, more technically, if
     *                                  {@code replacements} size % 2 is not equal to 0.
    ...
    /**
     * Gets the replacements being used by this {@link Replacer}. Modifying this list will have no effect, it can be
     * used for debugging or to create your own {@link Replacer} type while being able to {@link #clone()} it. This can
     * be done by calling the {@link #Replacer(Object...)} constructor.
     *
